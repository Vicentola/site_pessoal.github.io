...
moeda = [
    ...
    ["Nota de R$2", 2, 50],
    ["Moeda de R$1", 1, 50],
    ["Moeda de R$0,50", 0.50, 100],
    ["Moeda de R$0,25", 0.25, 100],
    ["Moeda de R$0,10", 0.10, 100],
    ["Moeda de R$0,05", 0.05, 100],
    ["Moeda de R$0,01", 0.01, 100]
]
...
def mostrar_menu():
    ...
    # o enumerate retorna o index da linha e a linha como uma lista, por isso e possivel chamar posicoes
        ...
        preco = produto[1]
        estoque = produto[2]
        print(f"{i + 1}. {produto[0]} - R$ {produto[1]} - Estoque: {estoque}") # serve para fornecer a informacao de cada produto na tela
...
def verificar_troco(troco):
    ...
    for i, item in enumerate(moeda): # percorre as moedas e faz as verificacoes
        ...
        if quantidade > estoque:
            quantidade = estoque  # Limitar a quantidade ao estoque disponível
            print("O troco não é suficiente!")
            break
        if quantidade > 0:
